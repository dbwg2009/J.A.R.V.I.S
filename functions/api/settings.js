...
export async function onRequestGet({ request, env }) {
  ...
  const row = await env.DB.prepare(
    `SELECT voice_on, web_search, system_prompt, llm_provider FROM settings WHERE user_id = ?`
  ...
  return json({
    ...
    llm_provider: row?.llm_provider || 'anthropic',
  });
}
...
export async function onRequestPut({ request, env }) {
  ...
  const { voice_on, web_search, system_prompt, llm_provider } = body;

  await env.DB.prepare(`
    INSERT INTO settings (user_id, voice_on, web_search, system_prompt, llm_provider, updated_at)
    VALUES (?, ?, ?, ?, ?, datetime('now'))
